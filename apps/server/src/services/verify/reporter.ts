 ...
 * contradict the rollup. Upserts per operation (regenerating overwrites).
 */
export class VerifyReporterService {
  ...
  async generateReport(params: GenerateReportParams) {
    ...
    if (results.length === 0) {
      log('generateReport: no results for op %s, skipping', operationId);
      return null;
    }

    const evidenceRows = await this.evidenceModel.listByOperation(operationId);
    ...
    return this.reportModel.upsertByOperation({
