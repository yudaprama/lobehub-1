...
export class VerifyExecutorService {
  ...
  async execute(params: ExecuteVerifyParams): Promise<void> {
    ...
    const toCreate = items
      ...
      .map((item) => planItemToPendingResult(params.operationId, item));
    ...
    const evidenceByItem = await this.loadEvidence(params.operationId);
    ...
    const gapIds = await this.runStructuralGate(params.operationId, items, evidenceByItem);
    ...
    await Promise.all([
      ...
      this.runLlmItems(params, llmItems, evidenceByItem),
  ...
  private async loadEvidence(operationId: string): Promise<EvidenceByItem> {
    const rows = await this.evidenceModel.listByOperation(operationId);
  ...
  private async runLlmItems(
    ...
    try {
      if (params.batchLlm ?? true) {
        await this.judgeBatch(params, items, evidenceByItem);
      } else {
        for (const item of items) await this.judgeSingle(params, item, evidenceByItem);
      }
    } catch (error) {
